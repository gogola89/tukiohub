...
**CRITICAL: Payment Status Polling Implementation**

⚠️ **Important Fix Applied**: The payment status polling must continue even when status is temporarily 'FAILED'.

**Why**: The M-Pesa callback flow works as follows:
1. STK Push is sent to user's phone
2. Backend initially may return `status: 'PENDING'` or even `status: 'FAILED'`
3. User enters PIN on their phone
4. Safaricom sends callback to backend (this may take 10-60 seconds)
5. Backend processes callback and updates status to 'COMPLETED'

**The Problem**: If polling stops when status is 'FAILED', the frontend will never see the status change to 'COMPLETED' after the callback is processed.

**The Solution**:
- Continue polling every 4-5 seconds regardless of status
- Only stop polling when:
  - Status is 'COMPLETED' (success!), OR
  - Timeout is reached (2 minutes default)
- Even if status is 'FAILED', keep polling until timeout
- Show timeout message only after 2 minutes of no success

**Implementation in `usePaymentStatus` hook**:
```typescript
export function usePaymentStatus(
  ...
  return useQuery({
    ...
    queryFn: () => paymentsAPI.checkPaymentStatus(transactionReference!),
    ...
    refetchInterval: (query) => {
      ...
      // 2. Timeout has been reached (via shouldStopPolling callback)
      ...
      // Backend might update from FAILED → COMPLETED after processing callback
...
- 0-30s: "Waiting for payment... Check your phone for M-Pesa prompt"
- 30-90s: Still showing "Processing..." even if backend says FAILED (waiting for callback)
- 90-120s: Continue waiting, show "Still processing..." message
- 120s+: Show timeout message with option to check booking status or retry

This ensures users who successfully complete payment on their phone will see the success status, even if there's a delay in callback processing.
